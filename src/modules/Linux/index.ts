...
export class Linux extends Computer {
	...
	async type(text: string): Promise<void> {
		const groups =
			text.match(new RegExp(`.{1,${Linux.typingGroupSize}}`, "g")) ?? [];
		for (const group of groups) {
			await this.command(
				`${this.xdotool} type --delay ${Linux.typingDelay} -- ${group}`
			);
		}
	}
	...
	async leftClick(): Promise<void> {
		await this.command(`${this.xdotool} click --repeat 1 --delay 100 1`);
	}

	async rightClick(): Promise<void> {
		await this.command(`${this.xdotool} click --repeat 1 --delay 100 3`);
	}

	async middleClick(): Promise<void> {
		await this.command(`${this.xdotool} click --repeat 1 --delay 100 2`);
	}

	async doubleClick(): Promise<void> {
		await this.command(`${this.xdotool} click --repeat 2 --delay 100 1`);
	}
	...
	async command(command: string): Promise<string> {
		const process = await Bun.spawn({
			cmd: [
				"bash",
				"-c",
				`docker exec -i ${Linux.containerName} sh -c "${command}"`,
			],
		});
